...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState<'overview' | 'products' | 'categories' | 'orders' | 'customers' | 'activation' | 'passwordReset' | 'ratings' | 'settings'>('overview');
  ...
  const tabs = [
    ...
    { key: 'categories', label: 'الأقسام', icon: <FolderTree size={18} /> },
    { key: 'orders', label: 'الطلبات', icon: <ClipboardList size={18} /> },
    { key: 'customers', label: 'العملاء', icon: <Users size={18} /> },
    { key: 'activation', label: 'طلبات التفعيل', icon: <UserCheck size={18} /> },
    { key: 'passwordReset', label: 'طلبات تغيير كلمة السر', icon: <KeyRound size={18} /> },
    { key: 'ratings', label: 'التقييمات', icon: <Star size={18} /> },
