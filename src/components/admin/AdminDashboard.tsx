...
import { supabase, ADMIN_PASSWORD, type Product, type Category } from '@/lib/supabase';
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-600 to-brand-800 p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md animate-fade-in">
          <div className="flex flex-col items-center mb-6">
            ...
            <h1 className="text-2xl font-extrabold text-gray-800">لوحة تحكم مكتبة المعلمين</h1>
          ...
          <a href="#/" className="block text-center mt-4 text-sm text-gray-500 hover:text-brand-600 transition">
            العودة للمتجر
          </a>
        </div>
      </div>
      ...
      <aside className="lg:w-64 bg-gray-900 text-white lg:min-h-screen shrink-0">
        <div className="p-5 flex items-center gap-3 border-b border-gray-800">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">مكتبة المعلمين</div>
          </div>
        </div>
        <nav className="p-3 space-y-1 flex lg:flex-col overflow-x-auto">
          <button
            onClick={() => setTab('overview')}
            className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold text-sm transition whitespace-nowrap ${
              tab === 'overview' ? 'bg-brand-600 text-white' : 'text-gray-300 hover:bg-gray-800'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold text-sm transition whitespace-nowrap ${
              tab === 'products' ? 'bg-brand-600 text-white' : 'text-gray-300 hover:bg-gray-800'
            }`}
          >
            <Package size={18} />
            المنتجات
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg font-semibold text-sm transition whitespace-nowrap ${
              tab === 'categories' ? 'bg-brand-600 text-white' : 'text-gray-300 hover:bg-gray-800'
            }`}
          >
            <FolderTree size={18} />
            الأقسام
        ...
        <div className="p-3 mt-auto hidden lg:block">
          ...
          <a
            href="#/"
            className="flex items-center gap-3 px-4 py-3 rounded-lg font-semibold text-sm text-gray-300 hover:bg-gray-800 transition w-full"
          >
            <LayoutDashboard size={18} />
            عرض المتجر
          </a>
                    ...
                    <tbody className="divide-y divide-gray-100 text-sm">
                      {filteredProducts.map((p) => (
                        <tr key={p.id} className="hover:bg-gray-50 transition">
                          ...
                          <td className="p-4 font-semibold text-gray-700">{p.price.toFixed(2)} ر.س</td>
                          <td className="p-4">
                            {p.discount_price && p.discount_price < p.price ? (
                              <span className="text-brand-600 font-bold">{p.discount_price.toFixed(2)} ر.س</span>
                            ) : (
                      ...
                      <div key={p.id} className="p-4 flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="font-bold text-sm text-gray-700">{p.price.toFixed(2)} ر.س</span>
                            {p.discount_price && p.discount_price < p.price && (
                              <span className="text-brand-600 font-bold text-sm">{p.discount_price.toFixed(2)}</span>
                            )}
                          </div>
