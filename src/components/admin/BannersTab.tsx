...
import { Image as ImageIcon, Plus, Trash2, ExternalLink, Video, MessageSquareText } from 'lucide-react';
...
export default function BannersTab() {
  ...
  async function addBanner() {
    if (contentType === 'text_ticker' ? !newTextContent.trim() : !newMediaUrl.trim()) return;
    setSaving(true);
    const maxOrder = banners.reduce((max, b) => Math.max(max, b.sort_order), 0);
    await supabase.from('store_banners').insert({
      content_type: contentType,
      image_url: contentType === 'text_ticker' ? null : newMediaUrl.trim(),
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className="text-sm text-gray-500 mb-6">
        صور، فيديوهات، أو شريط نص متحرك يظهر أعلى المنتجات في المتجر. تظهر الصور والفيديوهات بحجم يملأ المساحة (استريتش)، ويمكنك تحديد عرض وارتفاع مخصصين لكل إعلان.
      </p>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 mb-6 space-y-3">
        ...
        <div className="flex gap-2">
          {(['image', 'video', 'text_ticker'] as ContentType[]).map((t) => (
            <button
              key={t}
              type="button"
        ...
        <button
          onClick={addBanner}
          disabled={(contentType === 'text_ticker' ? !newTextContent.trim() : !newMediaUrl.trim()) || saving}
