...
import { Package, ChevronLeft, Clock, CheckCircle, Truck, Boxes, Hash, Gift } from 'lucide-react';
          ...
          {orders.map((order) => (
            <button key={order.id} onClick={() => viewOrder(order)} className="w-full bg-white rounded-2xl shadow-sm p-4 hover:shadow-md transition text-right flex items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{paymentLabels[order.payment_method]} — {formatPrice(order.grand_total)}</p>
              </div>
