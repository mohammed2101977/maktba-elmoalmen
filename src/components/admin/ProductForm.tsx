...
export default function ProductForm({
  ...
  function addCheckboxGroup() {
    setCheckboxGroups((prev) => [...prev, { id: uid(), name: '', min_selections: 0, max_selections: 0, items: [] }]);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const cleanedCheckboxes: CheckboxOption[] = checkboxOptions
      ...
      .map((c) => ({
        ...
        has_quantity: !!c.has_quantity,
        quantity_max: c.has_quantity ? Math.max(2, Number(c.quantity_max) || 10) : undefined,
      ...
      .map((g) => ({
        ...
        items: g.items
          ...
          .map((it) => ({
            ...
            has_quantity: !!it.has_quantity,
            quantity_max: it.has_quantity ? Math.max(2, Number(it.quantity_max) || 10) : undefined,
            default_checked: !!it.default_checked,
          })),
      }))
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="0"
            />
            <p className="text-xs text-gray-400 mt-1">تقييم المنتج بالنجوم أصبح حقيقياً الآن ويُحسب تلقائياً من تقييمات العملاء — يمكنك متابعته من قسم "التقييمات" في لوحة التحكم.</p>
          </div>
              ...
              {checkboxOptions.map((cb) => (
                <div key={cb.id} className="bg-gray-50 rounded-xl p-2 space-y-2">
                  ...
                  <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer pr-1">
                    ...
                    />
                    إظهار عداد "العدد" يضاعف القيمة حسب العدد المختار
                  </label>
                  {cb.has_quantity && (
                    <div className="flex items-center gap-2 pr-6">
                      <span className="text-xs text-gray-500">الحد الأقصى للعداد</span>
                      <input
                        type="number"
                        min="2"
                        value={cb.quantity_max ?? 10}
                        onChange={(e) => updateCheckboxOption(cb.id, { quantity_max: Math.max(2, parseInt(e.target.value) || 10) })}
                        className="w-20 h-8 rounded-lg border-2 border-gray-200 px-2 text-xs focus:outline-none focus:border-brand-500 transition"
                      />
                    </div>
                  )}
                  <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer pr-1">
                    <input
                      type="checkbox"
                      checked={!!cb.default_checked}
                      onChange={(e) => updateCheckboxOption(cb.id, { default_checked: e.target.checked })}
                      className="w-3.5 h-3.5 rounded accent-brand-600"
                    />
                    يظهر مُفعّلاً (نعم) افتراضياً دون زيادة في السعر — وعند إزالة العميل للعلامة يُخصم السعر
                  </label>
                </div>
              ))}
            </div>
                ...
                <div key={group.id} className="bg-gray-50 rounded-xl p-3 space-y-3">
                  <div className="flex flex-wrap gap-2 items-center">
                    <input
                      ...
                      className="flex-1 min-w-[140px] h-10 rounded-lg border-2 border-gray-200 px-3 text-sm focus:outline-none focus:border-brand-500 transition"
                    />
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-xs text-gray-500 whitespace-nowrap">أدنى اختيارات</span>
                      <input
                        type="number"
                        min="0"
                        value={group.min_selections}
                        onChange={(e) => updateCheckboxGroup(group.id, { min_selections: parseInt(e.target.value) || 0 })}
                        placeholder="0"
                        className="w-16 h-10 rounded-lg border-2 border-gray-200 px-2 text-sm focus:outline-none focus:border-brand-500 transition"
                      />
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                  ...
                  <p className="text-xs text-gray-400">0 في "أدنى" = بدون إلزام باختيار. 0 في "أقصى" = بدون حد أعلى.</p>
                      ...
                      <div key={item.id} className="bg-white rounded-lg p-2 space-y-2">
                        ...
                        {item.has_quantity && (
                          <div className="flex items-center gap-2 pr-6">
                            ...
                            <input
                              type="number"
                              min="2"
                              value={item.quantity_max ?? 10}
                              onChange={(e) => updateGroupItem(group.id, item.id, { quantity_max: Math.max(2, parseInt(e.target.value) || 10) })}
                              className="w-20 h-8 rounded-lg border-2 border-gray-200 px-2 text-xs focus:outline-none focus:border-brand-500 transition"
                            />
                          </div>
                        )}
                        <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer pr-1">
                          <input
                            ...
                            onChange={(e) => updateGroupItem(group.id, item.id, { default_checked: e.target.checked })}
                            className="w-3.5 h-3.5 rounded accent-brand-600"
                          />
                          يظهر مُفعّلاً (نعم) افتراضياً دون زيادة في السعر — وعند إزالة العميل للعلامة يُخصم السعر
                        </label>
