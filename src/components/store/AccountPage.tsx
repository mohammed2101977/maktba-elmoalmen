...
import { User, KeyRound, Package, ArrowRight, Printer, ChevronLeft, ShoppingBag } from 'lucide-react';
...
const paymentLabels: Record<string, string> = { instapay: 'الإنستا', vodafone: 'فودافون كاش', deposit: 'عربون (نصف الفاتورة)' };
...
export default function AccountPage() {
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-sm p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">سجّل دخولك لعرض طلباتك وإدارة حسابك</p>
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-1 text-sm">
              ...
              <div className="flex justify-between"><span className="text-gray-500">طريقة الدفع:</span><span className="font-semibold">{paymentLabels[selectedOrder.payment_method] ?? selectedOrder.payment_method}</span></div>
            ...
            <div className="space-y-3">
              {orderItems.map((item) => (
                <div key={item.id} className="flex items-center gap-3 bg-gray-50 rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0"><h4 className="font-bold text-sm text-gray-800 line-clamp-1">{item.product_name}</h4><p className="text-xs text-gray-500">{item.quantity} × {formatPrice(item.unit_price)}</p></div>
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {showPasswordModal && <AuthModal onClose={() => setShowPasswordModal(false)} onSuccess={() => setShowPasswordModal(false)} />}
    </div>
  );
}
