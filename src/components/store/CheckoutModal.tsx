...
import { X, CreditCard, Truck, Store, CheckCircle, MapPin, Gift, Hash } from 'lucide-react';
import { supabase, formatPrice, describeSelectedOptions, type StoreSettings } from '@/lib/supabase';
...
export default function CheckoutModal({
  ...
  const requiresTransferCode = paymentMethod === 'instapay' || paymentMethod === 'vodafone' || paymentMethod === 'deposit';
  ...
  async function handleConfirm() {
    ...
    if (requiresTransferCode && !transferCode.trim()) { setError('يرجى إدخال كود عملية التحويل لتأكيد الدفع'); return; }
    setLoading(true);
    try {
      ...
      const { data: orderData, error: orderError } = await supabase
        .from('orders').insert({
          ...
          grand_total: grandTotal, payment_method: paymentMethod, delivery_method: deliveryMethod,
          transfer_code: transferCode.trim() || null, status: 'reviewing',
        }).select('*').single();
      if (orderError || !orderData) { setError('حدث خطأ أثناء إنشاء الطلب'); setLoading(false); return; }

      const gifts: EarnedGift[] = [];
      const orderItems = cart.map((c) => {
        const giftQualifies = c.product.gift_enabled && c.qty >= c.product.gift_min_qty;
        if (giftQualifies) {
          gifts.push({
        ...
        return {
          ...
          gift_earned: giftQualifies ? (c.product.gift_description || 'هدية خاصة') : null,
        };
      });
              ...
              {earnedGifts.map((g, i) => (
                <div key={i} className="flex items-center gap-3 bg-white rounded-lg p-2">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-xs text-gray-500">هدية مع منتج: {g.productName}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              return (
                <div key={c.key} className="flex justify-between text-sm text-gray-600">
                  <span className="line-clamp-1">
                    ...
                    {c.product.gift_enabled && c.qty >= c.product.gift_min_qty && (
          ...
          {requiresTransferCode && (
            <div>
              ...
              <input
                ...
                placeholder="أدخل كود/رقم عملية التحويل بعد الدفع"
              />
              <p className="text-xs text-gray-400 mt-1">بعد تحويل المبلغ، أدخل كود العملية الذي وصلك لتأكيد الدفع.</p>
            </div>
          )}
          <div>
