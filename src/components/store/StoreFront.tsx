...
import { Search, ShoppingCart, Star, Menu, X, ChevronLeft, Heart, User, Phone } from 'lucide-react';
import {
  supabase,
  formatPrice,
  type Product,
...
export default function StoreFront() {
  ...
  function hasConfigurableOptions(p: Product) {
    return (p.option_groups?.length ?? 0) > 0 || (p.checkbox_options?.length ?? 0) > 0 || (p.counter_options?.length ?? 0) > 0;
  }
...
function ProductCard({
  ...
  const hasOptions = (product.option_groups?.length ?? 0) > 0 || (product.checkbox_options?.length ?? 0) > 0 || (product.counter_options?.length ?? 0) > 0;
