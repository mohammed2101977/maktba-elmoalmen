  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-6">
        {/* Ad banners */}
        {banners.length > 0 && (
          <div className="mb-8 -mx-3 sm:mx-0 px-3 sm:px-0 flex flex-wrap gap-3">
            {banners.map((banner) => {
              if (banner.content_type === 'text_ticker') {
                ...
                return (
                  <div
                    key={banner.id}
                    className="w-full rounded-2xl overflow-hidden bg-brand-600 py-3 flex"
              ...
              return (
                <div
                  key={banner.id}
                  className={`rounded-2xl overflow-hidden shadow-sm bg-gray-100 ${hasCustomWidth ? 'shrink-0' : 'w-full'} ${!banner.height ? 'aspect-[21/6] sm:aspect-[21/5]' : ''}`}
                  style={containerStyle}
                >
                  {banner.link_url && banner.content_type !== 'video' ? (
