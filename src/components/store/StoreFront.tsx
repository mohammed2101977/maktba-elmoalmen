...
import { Search, ShoppingCart, Star, Menu, X, ChevronLeft, Heart } from 'lucide-react';
...
export default function StoreFront() {
  ...
  const cartTotal = cart.reduce((sum, c) => sum + c.qty * (c.product.discount_price ?? c.product.price), 0);
  ...
  function getDiscountPercent(p: Product) {
    if (p.discount_price == null || p.discount_price >= p.price) return 0;
    ...
    <div className="min-h-screen bg-gray-100">
          ...
          <div className="flex items-center gap-3 h-16">
            ...
            <a href="#/" className="flex items-center gap-2 shrink-0">
              ...
              <div className="hidden sm:block">
                <div className="font-extrabold text-lg leading-tight">مكتبة المعلمين</div>
      ...
      <footer className="bg-gray-900 text-gray-300 mt-12">
        <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 sm:grid-cols-3 gap-8">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-extrabold text-white text-lg">مكتبة المعلمين</span>
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li><a href="#/admin" className="hover:text-brand-400 transition">لوحة التحكم</a></li>
            </ul>
          </div>
          <div>
        ...
        <div className="border-t border-gray-800 py-4 text-center text-sm text-gray-500">
          © 2026 مكتبة المعلمين. جميع الحقوق محفوظة.
        </div>
      </footer>
                    ...
                    <div key={item.product.id} className="flex gap-3 bg-gray-50 rounded-xl p-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-brand-600 font-bold text-sm">
                          {getEffectivePrice(item.product).toFixed(2)} ر.س
                        </p>
                        <div className="flex items-center gap-2 mt-1">
                          <button
                            onClick={() =>
                              setCart((prev) =>
                                prev.map((c) =>
                                  c.product.id === item.product.id ? { ...c, qty: Math.max(1, c.qty - 1) } : c
                                )
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              setCart((prev) =>
                                prev.map((c) => (c.product.id === item.product.id ? { ...c, qty: c.qty + 1 } : c))
                              )
                            }
                            className="w-7 h-7 rounded-md bg-gray-200 hover:bg-gray-300 font-bold text-sm"
                          >
            ...
            {cart.length > 0 && (
              <div className="border-t p-4 space-y-3">
                <div className="flex items-center justify-between font-bold text-lg">
                  <span>الإجمالي:</span>
                  <span className="text-brand-600">{cartTotal.toFixed(2)} ر.س</span>
                </div>
                <button className="w-full bg-brand-600 hover:bg-brand-700 text-white font-bold py-3 rounded-xl transition">
                  إتمام الطلب
                </button>
              </div>
            )}
