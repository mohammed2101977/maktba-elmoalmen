  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-6">
        {/* Ad banners */}
        {banners.length > 0 && (
          <div className="mb-8 -mx-3 sm:mx-0 px-3 sm:px-0 flex flex-wrap justify-end">
            {banners.map((banner) => {
              if (banner.content_type === 'text_ticker') {
                const tickerContent = (
                  <div
                    className="whitespace-nowrap inline-block animate-marquee font-bold text-white"
                    style={{ animationDuration: `${banner.ticker_speed_seconds ?? 20}s` }}
                  >
                    {banner.text_content}
                  </div>
                );
                return (
                  <div
                    key={banner.id}
                    className="w-full rounded-2xl overflow-hidden bg-brand-600 py-3 flex mb-3"
                    style={banner.height ? { height: `${banner.height}px`, alignItems: 'center' } : undefined}
                  >
                    {banner.link_url ? (
                      <a href={banner.link_url} target="_blank" rel="noopener noreferrer" className="w-full">
                        {tickerContent}
                      </a>
                    ) : (
                      tickerContent
              ...
              // Banners with a custom width sit side-by-side (wrapping to the next row only
              // when there's no more room); banners left at the default size take the full
              // row width, like before.
              const hasCustomWidth = !!banner.width;
              const containerStyle: React.CSSProperties = {};
              if (hasCustomWidth) containerStyle.width = `${banner.width}px`;
              if (banner.height) containerStyle.height = `${banner.height}px`;

              const media = banner.content_type === 'video' && banner.image_url
                ? (() => {
                    const embed = getVideoEmbed(banner.image_url as string);
                    return embed.kind === 'iframe' ? (
                      <iframe src={embed.src} className="w-full h-full" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen />
                    ) : (
                      <video src={embed.src} controls className="w-full h-full object-contain" />
                    );
                  })()
                : banner.content_type === 'slideshow'
                ? <BannerSlideshow slides={banner.slides ?? []} intervalMs={(banner.slideshow_interval_seconds ?? 3) * 1000} />
                : banner.image_url && <img src={banner.image_url} alt="" className="w-full h-full object-contain" />;

              return (
                <div
                  key={banner.id}
                  className={`rounded-2xl overflow-hidden shadow-sm bg-gray-100 mb-3 ${hasCustomWidth ? 'shrink-0' : 'w-full'} ${!banner.height ? 'aspect-[21/6] sm:aspect-[21/5]' : ''}`}
                  style={containerStyle}
                >
                  {banner.link_url && banner.content_type !== 'video' && banner.content_type !== 'slideshow' ? (
              ...
              {cart.length > 0 && generalGifts.length > 0 && (
                <div className="space-y-2 mb-4">
                  {generalGifts.map((gift) => {
                    ...
                    return (
                      <div key={gift.id} className={`rounded-xl border-2 p-3 flex gap-3 ${earned ? 'border-green-300 bg-green-50' : 'border-brand-100 bg-brand-50'}`}>
                        {gift.image_url && (
                    ...
                    return (
                      <div key={item.key} className="flex gap-3 bg-gray-50 rounded-xl p-3">
                        <img
                          ...
                          className="w-16 h-16 rounded-lg object-contain bg-gray-200"
                        />
