...
export type StoreBanner = {
  ...
  ticker_speed_seconds: number; // for 'text_ticker': seconds to scroll fully across the screen (lower = faster)
  slideshow_interval_seconds: number; // for 'slideshow': seconds each slide stays before auto-advancing
  active: boolean;
  sort_order: number;
  created_at: string;
};
...
// Formats a timestamp as an Arabic relative-time string, e.g. "منذ 5 دقائق".
export function timeAgoAr(dateStr: string): string {
  const diffMs = Date.now() - new Date(dateStr).getTime();
  const diffSec = Math.max(0, Math.floor(diffMs / 1000));

  const units: { limit: number; div: number; singular: string; dual: string; plural: string }[] = [
    { limit: 60, div: 1, singular: 'ثانية', dual: 'ثانيتين', plural: 'ثانية' },
    { limit: 3600, div: 60, singular: 'دقيقة', dual: 'دقيقتين', plural: 'دقائق' },
    { limit: 86400, div: 3600, singular: 'ساعة', dual: 'ساعتين', plural: 'ساعات' },
    { limit: 2592000, div: 86400, singular: 'يوم', dual: 'يومين', plural: 'أيام' },
    { limit: 31536000, div: 2592000, singular: 'شهر', dual: 'شهرين', plural: 'أشهر' },
    { limit: Infinity, div: 31536000, singular: 'سنة', dual: 'سنتين', plural: 'سنوات' },
  ];
  ...
  for (const u of units) {
    if (diffSec < u.limit) {
      const value = Math.max(1, Math.floor(diffSec / u.div));
      if (value === 1) return `منذ ${u.singular}`;
      if (value === 2) return `منذ ${u.dual}`;
      if (value >= 3 && value <= 10) return `منذ ${value} ${u.plural}`;
      return `منذ ${value} ${u.singular}`;
    }
  }
  return '';
}

// Formats a timestamp as an Arabic date + time string for display next to requests/orders.
