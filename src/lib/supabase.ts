...
export type CheckboxOption = {
  ...
  mode: 'add' | 'replace';
  price: number;
  has_quantity?: boolean; // if true, shows a 1-5 "العدد" stepper that multiplies the price
};
...
export type SelectedOptions = {
  ...
  checkboxQuantities: Record<string, number>; // checkbox/item id -> quantity (1-5), for has_quantity items
...
// its optional 1-5 "العدد" quantity multiplier.
function checkboxItemPrice(item: CheckboxOption, quantities: Record<string, number>): number {
  const qty = item.has_quantity ? Math.min(5, Math.max(1, quantities[item.id] ?? 1)) : 1;
...
export function describeSelectedOptions(product: Product, selected: SelectedOptions): { group: string; choice: string }[] {
  ...
  for (const cb of product.checkbox_options ?? []) {
    if (!selected.checkboxes.includes(cb.id)) continue;
    const qty = cb.has_quantity ? Math.min(5, Math.max(1, selected.checkboxQuantities[cb.id] ?? 1)) : null;
  ...
  for (const group of product.checkbox_groups ?? []) {
    ...
    for (const item of group.items) {
      if (!selectedIds.includes(item.id)) continue;
      const qty = item.has_quantity ? Math.min(5, Math.max(1, selected.checkboxQuantities[item.id] ?? 1)) : null;
